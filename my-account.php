...
if(strlen($_SESSION['login']) == 0){   
	...
	if(isset($_POST['update'])){
		...
		$query = $conn->query("update users set name='$name',contactno='$contactno' where id='".$_SESSION['id']."'");
		if($query)
		{
	...
	if(isset($_POST['submit'])){
		$sql = $conn->query("SELECT password FROM  users where password='".md5($_POST['cpass'])."' && id='".$_SESSION['id']."'");
		$num = mysqli_num_rows($sql);
		if($num > 0)
		{
		 $con = $conn->query("update students set password='".md5($_POST['newpass'])."', updationDate='$currentTime' where id='".$_SESSION['id']."'");
